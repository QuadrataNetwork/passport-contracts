...
const getRole = async (allowList: any, address: any) => {
  const role: BigNumber = await allowList.readAllowList(address);
...
task(
  "deployerAllowList:readRole",
  "a task to get the network deployer allow list"
)
...
task(
  "deployerAllowList:addDeployer",
  "a task to add the deployer on the allow list"
)
...
// npx hardhat allowList:addDeployer --network local --address [address]
task(
  "txAllowList:addDeployer",
  "a task to add an address to the transaction allow list"
)
  .addParam("address", "the address you want to add as a deployer")
  .setAction(async (args, hre) => {
    const allowList = await hre.ethers.getContractAt(
...
// npx hardhat allowList:addAdmin --network local --address [address]
task(
  "txAllowList:addAdmin",
  "a task to add a admin on the transaction allow list"
)
  .addParam("address", "the address you want to add as a admin")
  .setAction(async (args, hre) => {
    const allowList = await hre.ethers.getContractAt(
...
task("feeManager:get", "gets fee config").setAction(async (_, hre) => {
  const feeManager = await hre.ethers.getContractAt(
    "IFeeManager",
    FEE_MANAGER_ADDRESS
  );
