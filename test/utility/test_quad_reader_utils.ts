...
describe("QuadReaderUtils()", function () {
  ...
  describe("vantageScoreIteratorGreaterThan()", function () {
    it("asserts correct vantageScoreIteratorGreaterThan value", async () => {
      const startHash = ethers.utils.keccak256(
        ethers.utils.toUtf8Bytes("NeverFadeFaze")
      );
      ...
      const randomHash = ethers.utils.keccak256(
        ethers.utils.toUtf8Bytes("AlwaysFadeFaze")
      );

      expect(
        await testQuadReaderUtilsInstance.functions.vantageScoreIteratorGreaterThan(
          startHashPlus2,
          startHash,
          3
        )
