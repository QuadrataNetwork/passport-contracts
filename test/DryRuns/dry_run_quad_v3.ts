...
describe.skip("Dry Run Quad V3", () => {
  ...
  beforeEach(async () => {
    ...
    issuedAt =
      (await ethers.provider
        .getBlock("latest")
        .then((block: any) => block.timestamp)) - 1000;
    verifiedAt = issuedAt;
  });
  ...
  it("success - users[2] and users[3] issue attributes to users[0] and users[1]", async () => {
    ...
    const results = await reader.callStatic.getAttributes(
      users[0].address,
      ATTRIBUTE_IS_BUSINESS
    );
  ...
  it("fail - users cannot call passport.attributes", async () => {
    ...
    await expect(
      passport.attributes(users[0].address, ATTRIBUTE_IS_BUSINESS)
  ...
  it("fail - users cannot call passport.attribute", async () => {
    ...
    await expect(
      passport.attribute(users[0].address, ATTRIBUTE_IS_BUSINESS)
    ).to.be.revertedWith("INVALID_READER");
  });
  ...
  it("success - users[2] and users[3] issue 3 attributes to users[0])", async () => {
    ...
    const results = await reader.callStatic.getAttributesBulk(
      users[0].address,
      [ATTRIBUTE_IS_BUSINESS, ATTRIBUTE_COUNTRY, ATTRIBUTE_AML, ATTRIBUTE_DID]
    );

    expect(results[0].value).equals(id("FALSE"));
    expect(results[1].value).equals(id("UK"));
    expect(results[2].value).equals(hexZeroPad("0x03", 32));
    expect(results[3].value).equals(
      formatBytes32String("hello:world:candy:land")
    );

    const results2 = await reader.callStatic.getAttributes(
      users[0].address,
      ATTRIBUTE_IS_BUSINESS
    );
    ...
    const results3 = await reader.callStatic.getAttribute(
      users[0].address,
      ATTRIBUTE_COUNTRY
    );
    expect(results3.value).equals(id("UK"));

    const results4 = await reader.callStatic.getAttributesLegacy(
      users[0].address,
      ATTRIBUTE_IS_BUSINESS
    );
    ...
    const results5 = await reader.callStatic.getAttributesBulkLegacy(
      users[0].address,
      [ATTRIBUTE_IS_BUSINESS, ATTRIBUTE_COUNTRY, ATTRIBUTE_AML, ATTRIBUTE_DID]
    );
