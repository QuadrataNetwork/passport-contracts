  ...
  describe("mintPassport", async () => {
    ...
    it("fail - invalid hash (wrong DID)", async () => {
      ...
      await expect(
        passport
          .connect(minterA)
          .mintPassport(minterA.address, TOKEN_ID, wrongDID, aml, country, isBusiness, issuedAt, sig, {
            value: MINT_PRICE,
          })
    ...
    it("fail - invalid hash (wrong isBusiness)", async () => {
      ...
      await expect(
        passport
          .connect(minterA)
          .mintPassport(minterA.address, TOKEN_ID, did, aml, country, wrongIsBusiness, issuedAt, sig, {
            value: MINT_PRICE,
          })
      ).to.be.revertedWith("INVALID_ISSUER");
