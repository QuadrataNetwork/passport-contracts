...
import { id, parseUnits, formatBytes32String, hexZeroPad } from "ethers/lib/utils";
...
describe("QuadPassport", async () => {
  ...
  describe("burnPassportIssuer", async () => {
    ...
    it("success - mint business, update AML to 10, deactivate, reactivate, burn, assert AML is still 10", async () => {
      aml = hexZeroPad('0x0a', 32); // this is AML 10 as a bytes32 encoded hex
      isBusiness = id("TRUE");
      ...
      expect(await passport.balanceOf(mockBusiness.address, TOKEN_ID)).to.equal(1);
      await passport.connect(issuer).burnPassportIssuer(mockBusiness.address, TOKEN_ID);
      expect(await passport.balanceOf(mockBusiness.address, TOKEN_ID)).to.equal(0);
