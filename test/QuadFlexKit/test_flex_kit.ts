...
describe('QuadFlexKit', function() {
  ...
  describe('setAttributes()', function() {
    it('fails if issuer is not allowed', async () => {
      const attrKey = await flexKit.connect(issuer).getAttributeKey(issuer.address, ethers.utils.id('RANDOM'))

      await expect(
        flexKit.connect(issuer).setAttributes(
          attrKey,
    ...
    it('succeeds', async () => {
      ...
      await flexKit.connect(issuer).setAttributes(
        attrKey,
  ...
  describe('setRevokedAttributes()', function(){
    it('revoked and unrevokes attributes', async () => {
      ...
      // User revokes
      await flexKit.connect(treasury).setRevokedAttributes(
        attrKey,
        true
      )

      // Write should fail
      await expect(flexKit.connect(issuer).setAttributes(
        attrKey,
        ethers.utils.id('some-random-value-3'),
        treasury.address,
        sigAccount
  ...
  describe('queryFee()', function() {
    it('returns baseFee for single attributes no data', async () =>{
      await flexKit.connect(issuer).setQueryFee(ethers.utils.id('RANDOM'), baseFee)
      const attrKey = await flexKit.connect(issuer).getAttributeKey(issuer.address, ethers.utils.id('RANDOM'))

      const fee = await flexKit.connect(issuer).queryFee(
        issuer.address,
        attrKey
      );
    ...
    it('returns correct fee for primary attributes', async () =>{
      let fee = await flexKit.connect(issuer).queryFee(
        issuer.address,
        ethers.utils.id('COUNTRY')
      )
      ...
      fee = await flexKit.connect(issuer).queryFee(
        issuer.address,
  ...
  describe('queryFeeBulk()', function() {
    it('returns baseFee for single attributes', async () =>{
      ...
      const fee = await flexKit.connect(issuer).queryFeeBulk(
        issuer.address,
        [attrKey]
      );
    ...
    it('returns (baseFee * n) for multiple attributes', async () =>{
      await flexKit.connect(issuer).setQueryFee(ethers.utils.id('RANDOM'), baseFee)
      await flexKit.connect(issuer).setQueryFee(ethers.utils.id('RANDOM-2'), baseFee)
      await flexKit.connect(issuer).setQueryFee(ethers.utils.id('RANDOM-3'), baseFee)

      const attr1Key = await flexKit.connect(issuer).getAttributeKey(issuer.address, ethers.utils.id('RANDOM'))
      const attr2Key = await flexKit.connect(issuer).getAttributeKey(issuer.address, ethers.utils.id('RANDOM-2'))
      const attr3Key = await flexKit.connect(issuer).getAttributeKey(issuer.address, ethers.utils.id('RANDOM-3'))
    ...
    it('returns correct fee for primary attributes', async () =>{
      let fee = await flexKit.connect(issuer).queryFeeBulk(
        issuer.address,
        [ethers.utils.id('COUNTRY')]
      )
      ...
      fee = await flexKit.connect(issuer).queryFeeBulk(
        issuer.address,
        [ethers.utils.id('IS_BUSINESS')]
      )
      expect(fee.toString()).eql('0')
  ...
  describe('getAttributes()', function() {
    it('succeeds', async () =>{
      ...
      const fee = await flexKit.connect(issuer).queryFeeBulk(treasury.address, [attrKey])
      const attributes = await flexKit.connect(issuer).callStatic.getAttributes(treasury.address, attrKey, {value: fee})
      ...
      await flexKit.connect(issuer).getAttributes(treasury.address, attrKey, {value: fee})
    });
  });
  ...
  describe('withdraw()', function() {
    it('succeeds', async () => {
      ...
      await expect(flexKit.connect(issuer).withdraw()).to.be.revertedWith('CANNOT_WITHDRAW');
      await expect(flexKit.connect(treasury).withdraw()).to.be.revertedWith('CANNOT_WITHDRAW');
      await expect(flexKit.connect(admin).withdraw()).to.be.revertedWith('CANNOT_WITHDRAW');
      ...
      const fee = await flexKit.connect(issuer).queryFeeBulk(treasury.address, [attrKey]);
      await flexKit.connect(issuer).getAttributesLegacy(treasury.address, attrKey, {value: fee});

      // Expect funds to be set properly
      expect(await flexKit.connect(issuer).funds(treasury.address)).eq(quadrataFee);
    ...
    it('succeeds with legacy attributes', async () => {
      ...
      const fee1 = await flexKit.connect(issuer).queryFeeBulk(treasury.address, [ethers.utils.id('COUNTRY')]);
      await flexKit.connect(issuer).getAttributesLegacy(treasury.address, ethers.utils.id('COUNTRY'), {value: fee1});
    ...
    it('succeeds with no attributes posted', async () => {
      ...
      //First query
      const fee1 = await flexKit.connect(issuer).queryFeeBulk(treasury.address, [attrKey]);
      await flexKit.connect(issuer).getAttributesLegacy(treasury.address, attrKey, {value: fee1});
    ...
    it('succeeds with multiple queries', async () => {
      ...
      //Second query
      const fee2 = await flexKit.connect(issuer).queryFeeBulk(treasury.address, [attrKey]);
      await flexKit.connect(issuer).getAttributesLegacy(treasury.address, attrKey, {value: fee2});

      // Expect funds to be set properly
      expect(await flexKit.connect(issuer).funds(treasury.address)).eq(BigNumber.from(quadrataFee).mul(2).toString());
      expect(await flexKit.connect(issuer).funds(issuer.address)).eq(baseFee.toString());

      //Third legacy query
      const fee3 = await flexKit.connect(issuer).queryFeeBulk(treasury.address, [ethers.utils.id('COUNTRY')]);
      await flexKit.connect(issuer).getAttributesLegacy(treasury.address, ethers.utils.id('COUNTRY'), {value: fee3});
  ...
  describe('setQueryFee()', function() {
    it('succeeds', async () => {
      const attrKey = await flexKit.connect(issuer).getAttributeKey(issuer.address, ethers.utils.id('RANDOM-RAW-ATTR'))
      ...
      await flexKit.connect(issuer).setQueryFee(ethers.utils.id('RANDOM-RAW-ATTR'), baseFee)

      const feeAfter = await flexKit.connect(issuer).queryFeeBulk(treasury.address, [attrKey])
  ...
  describe('getAttributesLegacy()', function() {
    it('succeeds', async () =>{
      ...
      const fee = await flexKit.connect(issuer).queryFeeBulk(treasury.address, [attrKey])
      const attributes = await flexKit.connect(issuer).callStatic.getAttributesLegacy(treasury.address, attrKey, {value: fee})
  ...
  describe('getAttributesBulk()', function() {
    it('succeeds', async () => {
      ...
      const fee = await flexKit.connect(issuer).queryFeeBulk(treasury.address, [attrKey])
      const attributes = await flexKit.connect(issuer).callStatic.getAttributesBulk(treasury.address, [attrKey], {value: fee})

      expect(attributes.length).eql(1)
      expect(attributes[0].value).eql(ethers.utils.id('some-random-value'))
      expect(attributes[0].issuer).eql(issuer.address)

      await flexKit.connect(issuer).getAttributesBulk(treasury.address, [attrKey], {value: fee})
    });
    ...
    it('succeeds with multiple attributes', async () => {
      ...
      const attrKey2 = await flexKit.connect(issuer).getAttributeKey(issuer.address, ethers.utils.id('RANDOM-2'))
      await flexKit.connect(issuer).setQueryFee(ethers.utils.id('RANDOM-2'), baseFee)

      const attrKey3 = await flexKit.connect(issuer).getAttributeKey(issuer.address, ethers.utils.id('RANDOM-3'))
      await flexKit.connect(issuer).setQueryFee(ethers.utils.id('RANDOM-3'), baseFee)
      ...
      const fee = await flexKit.connect(issuer).queryFeeBulk(treasury.address, [attrKey, attrKey2, attrKey3])
      const attributes = await flexKit.connect(issuer).callStatic.getAttributesBulk(treasury.address, [attrKey, attrKey2, attrKey3], {value: fee})
      ...
      await flexKit.connect(issuer).getAttributesBulk(treasury.address, [attrKey, attrKey2, attrKey3], {value: fee})
    })
    ...
    it('fails with not enough eth', async () => {
      ...
      const fee = await flexKit.connect(issuer).queryFeeBulk(treasury.address, [attrKey])
      await expect(
        flexKit.connect(issuer).getAttributesBulk(treasury.address, [attrKey], {value: 0})
      ).to.be.revertedWith('INVALID_FEE');

      await expect(
        flexKit.connect(issuer).getAttributesBulk(treasury.address, [attrKey], {value: fee + 100})
      ).to.be.revertedWith('INVALID_FEE');
    });
  });
  ...
  describe('getAttributesBulkLegacy()', function() {
    it('succeeds', async () =>{
      ...
      const fee = await flexKit.connect(issuer).queryFeeBulk(treasury.address, [attrKey])
      const attributes = await flexKit.connect(issuer).callStatic.getAttributesBulkLegacy(treasury.address, [attrKey], {value: fee})
    ...
    it('fails with not enough eth', async () => {
      ...
      const fee = await flexKit.connect(issuer).queryFeeBulk(treasury.address, [attrKey])
      await expect(
        flexKit.connect(issuer).getAttributesBulkLegacy(treasury.address, [attrKey], {value: 0})
      ).to.be.revertedWith('INVALID_FEE');

      await expect(
        flexKit.connect(issuer).getAttributesBulkLegacy(treasury.address, [attrKey], {value: fee + 100})
      ).to.be.revertedWith('INVALID_FEE');
    });
  });
});
