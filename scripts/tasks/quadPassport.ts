...
task(
  "getSetReaderFunctionData",
  "npx hardhat getSetReaderFunctionData --reader <address>"
)
  ...
  .setAction(async function (taskArgs, hre) {
    ...
    if (hre.network.name != "hardhat") {
      console.log(
        "getSetReaderFunctionData can only be run on hardhat network"
      );
      return;
    }
    ...
    const setQuadReaderFunctionData = passport.interface.encodeFunctionData(
      "setQuadReader",
      [address]
    );
...
task("getAllMinters", "npx hardhat getAllMinters --passport <address>")
  ...
  .setAction(async function (taskArgs, hre) {
    ...
    const filter = passport.filters.TransferSingle(
      null,
      null,
      null,
      null,
...
task(
  "setAttributes",
  "npx hardhat setAttributes --passport <address> --account <address> --did <bytes32> --raw <flag> --attributes <string,string,...> --values <string,string,...> --network <network>"
)
  ...
  .setAction(async function (taskArgs, hre) {
    ...
    for (const attrKey of attrTypes) {
      const eligibilityByDid = await governance.eligibleAttributesByDID(
        attrKey
      );
      ...
      const issuerHasPermission = await governance.getIssuerAttributePermission(
        issuer.address,
        attrKey
      );
    ...
    for (var i = 0; i < attrTypes.length; i++) {
      if (attrTypes[i] === id("AML")) {
        ...
        const formattedAML = hexZeroPad(
          ethers.BigNumber.from(values[i]).toHexString(),
          32
        );
    ...
    const tx = await passport
      ...
      .setAttributesIssuer(
        minter,
        [attrKeys, attrValues, attrTypes, did, 1, verifiedAt, issuedAt, 0],
        sigIssuer,
        {
          value: 0,
          gasLimit: 8000000,
...
task(
  "getAllMintersByTokenIds",
  "npx hardhat getAllMintersByTokenIds --passport <address> --tokenIds <string,string,...>"
)
  .addParam("passport", "sets the address for passport")
