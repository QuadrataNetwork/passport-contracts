...
task(
  "getAttributes",
  "npx hardhat getAttributes --reader <address> --account <address> --attribute <string> --blockNumber <number> "
)
  ...
  .setAction(async function (taskArgs, hre) {
    ...
    const quadReader = await recursiveRetry(
    ...
    const results = await recursiveRetry(async () => {
      ...
      const queryFee = await quadReader.callStatic.queryFee(
        accountAddress,
        ethers.utils.id(attribute),
        { blockTag: blockNumber }
      );
      console.log("query fee: " + queryFee);

      console.log("attempting to read attributes");
      return await quadReader.callStatic.getAttributes(
        accountAddress,
        ethers.utils.id(attribute),
        {
          value: queryFee,
...
task(
  "assertAllAttributesEqual",
  "npx hardhat assertAllAttributesEqual --reader <address> --accounts <address,address,...> --attributes <string,string,...>  --startBlock <number> --endBlock <number> "
)
  .addParam("reader", "sets the address for reader")
    ...
    for (var i = 0; i < accounts.length; i++) {
      for (var j = 0; j < attributes.length; j++) {
        ...
        const results1 = await recursiveRetry(async () => {
          ...
          return await quadReader.callStatic.getAttribute(
            account,
            ethers.utils.id(attribute),
            {
              value: queryFee,
...
task(
  "getAllQueries",
  "npx hardhat getAllQueries --reader <address> --startBlock <number>"
)
  .addParam("reader", "sets the address for reader")
  .addParam("startBlock", "sets the start block")
  .setAction(async function (taskArgs, hre) {
    ...
    for (var i = startBlock; i < currentBlockNumber; i += stepSize) {
      ...
      for (const log in logs) {
        ...
        callers.add(_caller);
        callerToEventCount.set(
          _caller,
          (callerToEventCount.get(_caller) || 0) + 1
        );
      }
      ...
      console.log(
        "progress: " + Math.floor((i / currentBlockNumber) * 100) + "%"
      );
    }
...
task(
  "getAllAccountsQueried",
  "npx hardhat getAllAccountsQueried --reader <address> --startBlock <number>"
)
  .addParam("reader", "sets the address for reader")
  .addParam("startBlock", "sets the start block")
  .setAction(async function (taskArgs, hre) {
    ...
    for (var i = startBlock; i < currentBlockNumber; i += stepSize) {
      ...
      for (const log in logs) {
        ...
        accountsQueriedEventCount.set(
          _account,
          (accountsQueriedEventCount.get(_account) || 0) + 1
        );
      }
