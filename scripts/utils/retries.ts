...
export const recursiveRetry = async (
  func: (...args: any) => any,
  ...args: any
): Promise<any> => {
  try {
    ...
    if (
      e.toString().includes("cannot estimate gas") ||
      e.toString().includes("UNPREDICTABLE_GAS_LIMIT")
    ) {
      // dispatch to recursiveRetryErrorHandling
      return await recursiveRetryErrorHandling(
        func,
        {
...
export const recursiveRetryErrorHandling = async (
  func: (...args: any) => any,
  ...
  try {
    ...
    if (
      ...
      e.toString().includes("exceeds block gas limit") ||
      e.toString().includes("> current max gas")
    ) {
      ...
      console.log(
        "mannually discovering gas limit from error message to be " +
          targetGasLimit.toString()
      );
...
export const recursiveRetryIncreamentNonce = async (
