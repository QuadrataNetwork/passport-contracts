...
/// @notice All accessor functions for reading and pricing quadrata attributes
 ...
 contract QuadReaderOld is IQuadReaderOld, UUPSUpgradeable, ReentrancyGuardUpgradeable, QuadReaderStoreOld {
    ...
    function initialize(
        ...
        governance = IQuadGovernanceOld(_governance);
        passport = IQuadPassportOld(_passport);
    }
    ...
    function _includedIssuers(
        ...
        for(uint256 i = 0; i < issuers.length; i++) {
            if(governance.getIssuerStatus(_issuers[i]) == QuadGovernanceStoreOld.IssuerStatus.DEACTIVATED) {
                issuers[i] = address(0);
                gaps++;
            }
        }
    ...
    function _excludedIssuers(
        ...
        QuadGovernanceStoreOld.Issuer[] memory issuerData = governance.getIssuers();
        address[] memory issuers = new address[](governance.getIssuersLength());

        uint256 gaps = 0;
        for(uint256 i = 0; i < issuers.length; i++) {
            if(issuerData[i].status == QuadGovernanceStoreOld.IssuerStatus.DEACTIVATED) {
                gaps++;
                continue;
            }
    ...
    function _applyFilter(
        ...
        for(uint256 i = 0; i < _issuers.length; i++) {
            if(governance.eligibleAttributes(_attribute)) {
                ...
                QuadPassportStoreOld.Attribute memory dID = passport.attributes(_account,keccak256("DID"), _issuers[i]);
        ...
        QuadPassportStoreOld.Attribute memory attribute;
        for(uint256 i = 0; i < _issuers.length; i++) {
    ...
    function _isDataAvailable(
        ...
        QuadPassportStoreOld.Attribute memory attrib = passport.attributes(_account, _attribute, _issuer);
    ...
    function _isDataAvailableByDID(
        ...
        QuadPassportStoreOld.Attribute memory attrib = passport.attributesByDID(_dID, _attribute, _issuer);
        return attrib.value != bytes32(0) && attrib.epoch != 0;
    }
